...
import {barFullNameMap, filterToWordMap, initialEventSliceState} from "../../shared/constants";
...
export const eventSlice = createSlice({
    ...
    reducers: {
        ...
        setBarFilter: (state, action: PayloadAction<BarFilter>) => {
            state.events = [...state.initialApiResponse.events] as TEvent[];
        ...
        filterBySearchResult: (state, action: PayloadAction<string>) => {
            ...
            if (action.payload.length > 0) state.events = state.events.filter(
                event => event.name.toLowerCase().includes(action.payload)
            )
        }
    },
...
export const { setFilter, setBarFilter, filterBySearchResult } = eventSlice.actions;
